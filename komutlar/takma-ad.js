...
if(!kullanici) return message.reply(`Lütfen bir kullanıcı giriniz! \nDoğru Kullanım; \`${prefix}takma-ad @${client.user.username}#${client.user.discriminator} <yeni isim>\``)
...
message.channel.send(`Başarılı bir şekilde \`${kullanici.username}\` adlı kişinin takma adı \`${isim}\` olarak değiştirildi.`)
}
