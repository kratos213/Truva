...
let { oldu, hata, prefix, z } = require("../ayarlar.json")
module.exports.run = async (client, message, args) => {
let banyetkili =  await db.fetch(`banyetkili.${message.guild.id}`)
let banlog = await db.fetch(`banlog.${message.guild.id}`)

if(!banyetkili) return message.channel.send(new Discord.MessageEmbed().setDescription(`Ban Yetkili Rolü Ayarlanmamış! : ${prefix}ban-yetkili-rol @rol`).setColor(hata))
